 ...
 *
 * Description: If the file isn't an ELF file then exit code with 98.
 */
void check_elf(unsigned char *e_ident)
{
	int index;
...
/**
 * print_magic - Prints the mag numbrs of an ELF header.
 * @e_ident: A pointer to an array that contains the ELF magic numbers.
 *
 * Description: Magic numbers are seprtd by spaces.
 */
void print_magic(unsigned char *e_ident)
{
	int index;
...
/**
 * print_type - The type of the ELF header is printed.
 * @e_type: The ELF type.
 ...
 * @argc: The num of arguments available in the program.
